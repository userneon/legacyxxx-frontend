...
export function ReviewCardsSkeleton({ count, className }: { count: number; className?: string }) {
  return (
    <div className={cn("grid gap-3", className)} aria-hidden="true">
      {Array.from({ length: count }, (_, index) => (
        <div key={index} className="glass rounded-2xl p-4">
          <div className="flex items-center gap-3">
            <Skeleton className={cn("size-10 rounded-md", bar)} />
            <div className="space-y-1.5">
              <Skeleton className={cn("h-3.5 w-28", bar)} />
              <Skeleton className={cn("h-3 w-20", bar)} />
            </div>
          </div>
          <Skeleton className={cn("mt-3 h-20 w-full rounded-lg", bar)} />
        </div>
      ))}
    </div>
  )
