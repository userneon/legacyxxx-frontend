...
/* Neutral team tones: brightness tells the teams apart, no hue (design spec §2). */
const TEAM_TONE = {
  team1: { text: "text-[var(--text)]", bar: "bg-[var(--accent-solid)]" },
  team2: { text: "text-[var(--text-muted)]", bar: "bg-[var(--text-dim)]" },
} as const

/* Round tiles carry the side that won them: muted yellow T, sky-blue CT. */
const SIDE_TONE = { t: "text-[var(--team-t)] bg-[var(--team-t)]/15 border-[var(--team-t)]/40", ct: "text-[var(--team-ct)] bg-[var(--team-ct)]/15 border-[var(--team-ct)]/40" } as const
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-white/[0.07] bg-white/[0.02]">
      <div className="flex items-center justify-between gap-3 border-b border-white/[0.06] px-3 py-2.5">
        ...
        <span className={cn("rounded-md border px-2 py-0.5 text-[11px] font-bold uppercase", team.won ? "border-[var(--result-win)]/40 bg-[var(--result-win)]/15 text-[var(--result-win)]" : "border-[var(--result-loss)]/35 bg-[var(--result-loss)]/10 text-[var(--result-loss)]")}>
        ...
        {detail.teams.map((team) => (
          <div key={team.key} className="flex items-center justify-between rounded-lg bg-white/[0.03] px-3 py-2 text-xs">
            ...
            <span className="flex gap-3 text-muted-foreground">
              <span className="inline-flex items-center gap-1.5"><TeamIcon side="ct" /><span className="font-semibold text-[var(--team-ct)]">{sideWins(team.key, "ct")}</span></span>
              <span className="inline-flex items-center gap-1.5"><TeamIcon side="t" /><span className="font-semibold text-[var(--team-t)]">{sideWins(team.key, "t")}</span></span>
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div className="flex h-1.5 gap-0.5 overflow-hidden rounded-full bg-white/[0.06]">
        ...
        <div className="match-compare-bar h-full flex-1 rounded-r-full bg-[var(--text-dim)]" />
      </div>
    </div>
  )
}
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_260px]">
      <div className="stagger-in flex flex-col gap-3.5 rounded-xl border border-white/[0.07] bg-white/[0.02] p-4">
        <div className="flex items-center justify-between text-xs font-semibold">
          <span className="text-[var(--text)]">{team1.name}</span>
          <span className="text-[var(--text-muted)]">{team2.name}</span>
        </div>
            ...
            {team1 && team2 && (
              <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                ...
                <div className="min-w-0 text-left">
                  ...
                  {team2.won && <div className="mt-0.5 inline-flex items-center gap-1 text-[10px] font-bold uppercase text-[var(--result-win)]"><Medal className="size-3" />Winner</div>}
                </div>
              </div>
            )}
          </div>
