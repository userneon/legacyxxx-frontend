...
export const staffPanelService = {
  ...
  createStaff: (input: Pick<StaffPanelMember, "userId" | "role" | "permissions" | "status">, options?: CallOptions) => post<StaffPanelMember>("/staffpanel/staff", input, options),
  updateStaff: (staffId: string, input: Partial<Pick<StaffPanelMember, "role" | "permissions" | "status">>, options?: CallOptions) => put<StaffPanelMember>(`/staffpanel/staff/${encodeURIComponent(staffId)}`, input, options),
